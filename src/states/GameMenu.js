...
class GameMenu {
    ...
    create() {
        ...
        this.version = game.add.text(game.world.width - 64, game.world.height - 48, "0.10.0", styles.customText('10pt'));

        utils.navItemSetter('Start', 2, 60, target => game.state.start('TheGame') ); // go to History instead;
        utils.navItemSetter('Instructions', 3, 60, target => game.state.start('Instructions') );
        utils.navItemSetter('Options', 4, 60, target => game.state.start('Options') );
        utils.navItemSetter('Credits', 5, 60, target => game.state.start('Credits') );
